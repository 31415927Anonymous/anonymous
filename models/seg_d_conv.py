...
def placeholder_inputs(batch_size, num_point, use_normal=True):
    ...
    return pointclouds_pl, labels_pl, direc_pl, cls_pl


def get_model(point_cloud, direction, cls_label, is_training, bn_decay=None, use_normal=True, k=2):
    '''
    ...
    l0_points = tf_util.conv1d(l0_points, 64, 1, padding='VALID', bn=True, is_training=is_training, scope='fc0_1',
                               bn_decay=bn_decay)
    l1_points, l1_xyz, l1_dir = conv_dir_module(l0_xyz, direction, l0_points, npoint=512, radius=0.1, mlp=[64*i],
    ...
    l2_points, l2_xyz, l2_dir = conv_res_module(l1_xyz, l1_dir, l1_points, radius=0.15, mlp=[64*i],
                                                conv=[64*i], mlp2=[128*i], resnum=5, is_training=is_training,
                                                bn_decay=bn_decay, scope='layer2', kernel_size=7, bn=True,
                                                use_nchw=True, center=True)
    l3_points, l3_xyz, l3_dir = conv_dir_module(l2_xyz, l2_dir, l2_points, npoint=128, radius=0.2, mlp=[64*i],
                                                conv=[64*i], mlp2=[128*i], is_training=is_training,
                                                bn_decay=bn_decay, scope='layer3', kernel_size=7, bn=True,
                                                use_nchw=True, center=True)
    l4_points, l4_xyz, l4_dir = conv_res_module(l3_xyz, l3_dir, l3_points, radius=0.2, mlp=[64*i],
                                                conv=[64*i], mlp2=[128*i], resnum=5, is_training=is_training,
    ...
    l4_points_d = tf.tile(tf.reshape(l5_points, [batch_size, 1, -1]), [1, np, 1])
    ...
    l4_points_d = conv_decode_module(l4_xyz, l3_xyz, l3_dir, l3_points, l4_points_d, radius=0.2, mlp=[512*i, 256*i], conv=[128*i],
                                     ...
                                     kernel_size=7, bn=True, use_xyz=True, use_nchw=False, center=True)
    l3_points_d, _, _ = conv_res_module(l3_xyz, l3_dir, l4_points_d, radius=0.2, mlp=[64 * i], conv=[64 * i],
                                        mlp2=[128 * i], resnum=5, is_training=is_training, bn_decay=bn_decay,
                                        scope='dlayer3', kernel_size=7, bn=True, use_nchw=True, center=True)
    l2_points_d = pointnet_fp_module(l2_xyz, l3_xyz, tf.concat([l2_points, l1_points], axis=2), l3_points_d, [],
                                     is_training, bn_decay, scope='fp_layer2')
    l2_points_d = conv_decode_module(l2_xyz, l1_xyz, l1_dir, l1_points, l2_points_d, radius=0.15, mlp=None, conv=[128*i],
                                     mlp2=None, is_training=is_training, bn_decay=bn_decay, scope='dlayer2', use_new=False,
                                     kernel_size=7, bn=True, use_xyz=True, use_nchw=False, center=True)
    l1_points_d, _, _ = conv_res_module(l1_xyz, l1_dir, l2_points_d, radius=0.15, mlp=[64*i],
                                        conv=[64*i], mlp2=[128*i], resnum=5, is_training=is_training,
    ...
    l0_points_d = pointnet_fp_module(l0_xyz, l1_xyz, l0_points, l1_points_d, [], is_training, bn_decay,
                                     scope='fp_layer1')
    cls_label_one_hot = tf.one_hot(cls_label, depth=16, on_value=1.0, off_value=0.0)
    cls_label_one_hot = tf.reshape(cls_label_one_hot, [batch_size, 1, 16])
    cls_label_one_hot = tf.tile(cls_label_one_hot, [1,num_point,1])
    l0_points_d = tf.concat([cls_label_one_hot, l0_xyz, l0_points_d], axis=-1)
    # FC layers
...
def focal_loss(cls_weight, gamma=2, balance=False):
    def FL(pred, label):
        ...
        label_one_hot = tf.one_hot(label, depth=50, on_value=1.0, off_value=0.0)   # BxNx50
...
def get_focal_loss(pred, label, weight=[1.0]*50):
    """ pred: BxNxC,
        label: BxN, """
