...
def main():
    ...
    indices_extra = tf.placeholder(tf.int32, shape=(None, None, 2), name="indices_extra")
    ...
    sampled_indices = tf.placeholder(tf.int32, shape=(None, None, 2), name="sampled_indices")

    pts_fts = tf.placeholder(tf.float32, shape=(None, point_num, setting.data_dim), name='pts_fts')
    direc_pl = tf.placeholder(tf.float32, shape=(None, point_num, 6), name='direction')
    ...
    pts_fts_extra = tf.placeholder(tf.float32, shape=(None, point_num_extra, setting.data_dim), name='pts_fts')
    direc_pl_extra = tf.placeholder(tf.float32, shape=(None, point_num_extra, 6), name='direction')
    labels_seg_extra = tf.placeholder(tf.int64, shape=(None, point_num_extra), name='labels_seg')

    ######################################################################
    pts_fts_sampled = tf.gather_nd(pts_fts, indices=indices, name='pts_fts_sampled')
    pts_fts_extra_sampled = tf.gather_nd(pts_fts_extra, indices=indices_extra, name='pts_fts_extra_sampled')
    pts_fts_sampled = tf.concat([pts_fts_sampled, pts_fts_extra_sampled], axis=1)
    ...
    dir_sampled = tf.gather_nd(direc_pl, indices=indices, name='direction_sampled')
    dir_sampled_extra = tf.gather_nd(direc_pl_extra, indices=indices_extra, name='direction_sampled_extra')
    ...
    labels_sampled_extra= tf.gather_nd(labels_seg_extra, indices=indices_extra, name='labels_sampled_extra')
    labels_weights_sampled = tf.gather_nd(labels_weights, indices=indices, name='labels_weight_sampled')

    bn_exp_op = tf.train.exponential_decay(0.5, global_step, setting.decay_steps,
                                           0.5, staircase=True)
    bn_clip_op = tf.minimum(1 - bn_exp_op, 0.99)

    net = model.get_model(tf.concat([points_augmented, features_augmented], axis=-1), dir_augmented, is_training,
                          bn_decay=bn_clip_op)
        ...
        for batch_idx_train in range(batch_num):
            ...
            if (batch_idx_train % step_val == 0 and (batch_idx_train != 0 or args.load_ckpt is not None)) \
                ...
                for batch_val_idx in range(batch_num_val):
                    ...
                    points_num_batch_extra = data_num_val_extra[start_idx:end_idx, ...]
                    labels_batch_extra = label_val_extra[start_idx:end_idx, ...]
                    direc_batch_extra = direc_val_extra[start_idx:end_idx, ...]
                    
                    ind0 = np.tile(np.reshape(np.arange(batch_size_val), [-1, 1, 1]), [1, sample_num, 1])
                    ind1 = np.tile(np.reshape(np.arange(sample_num), [1, -1, 1]), [batch_size_val, 1, 1])
                    ...
                    sess.run([loss_mean_update_op, t_1_acc_update_op, t_1_per_class_acc_update_op],
                             feed_dict={
                                 ...
                                 direc_pl: direc_batch,
                                 indices: pf.get_indices(batch_size_val, sample_num, points_num_batch),
                                 pts_fts_extra: points_batch_extra,
                                 direc_pl_extra: direc_batch_extra,
                                 indices_extra: pf.get_indices(batch_size_val, sample_num_extra, points_num_batch_extra),
                                 xforms: xforms_np,
                                 rotations: rotations_np,
                                 jitter_range: np.array([jitter_val]),
                                 labels_seg: labels_batch,
                                 labels_seg_extra: labels_batch_extra,
                                 labels_weights: weights_batch,
                                 is_training: False,
                                 sampled_indices: sampled_indices_batch,
                             })
            ...
            ind0 = np.tile(np.reshape(np.arange(batch_size_train), [-1, 1, 1]), [1, sample_num_train, 1])
            ind1 = np.tile(np.reshape(np.arange(sample_num_train), [1, -1, 1]), [batch_size_train, 1, 1])
            sampled_indices_batch = np.concatenate([ind0, ind1], axis=-1)
            ...
            sess.run([train_op, loss_mean_update_op, t_1_acc_update_op, t_1_per_class_acc_update_op],
                     feed_dict={
                         ...
                         pts_fts_extra: points_batch_extra,
                         direc_pl_extra: direc_batch_extra,
                         indices_extra: pf.get_indices(batch_size_train, sample_num_train_extra, points_num_batch_extra),
                         xforms: xforms_np,
                         rotations: rotations_np,
